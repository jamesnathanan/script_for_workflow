...
const addMetadata = (_dna, _edition) => {
  ...
  let meta_data = {}
  meta_data.name = `${NFT_DETAILS.namePrefix} #${_edition}`
  meta_data.attributes = attributesList
  let tempMetadata = {
    id: `${NFT_DETAILS.namePrefix} #${_edition}`,
    meta: meta_data,
...
const writeMetaData = (_data) => {
  ...
  let file1Raw = fs.readFileSync(`${FOLDERS.jsonDir}/_metadata.json`);
  ...
  file1.forEach((element) => {
    ...
    fs.writeFile(`${FOLDERS.jsonDir}/${edition}.json`, writeData, (err) => {
      if (err) throw err;
